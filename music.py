...
class MusicPlayer(commands.Cog, name='Music'):
    ...
    @command(name='repeat', aliases=['rep'])
    ...
    @commands.has_permissions(manage_channels=True)
    @command(name='resume',aliases=['cont'])
    ...
    # @commands.command(brief='NO DATABASE will give error', description='[prefix]download <video url or title> Downloads the song')
    ...
    #             embed = discord.Embed(
    #                 title="Your download is ready", description="Please wait a moment while the file is being uploaded")
